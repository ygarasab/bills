...
const getBillInfo = writtenLine =>
{
    ...
    const barCode = line.substr(0,4) + line.slice(32) + fields[0].value.slice(4), fields[1].value, fields[2].value
    const vd = barCode.charAt(4)

    const verifiable = barcode.slice(4) + barcode.substr(5,barCode.length)
    if(!verificationDigitIsValid(barCode, vd, 11))
...
{
    let fields = []
    fields.push({value: line.substr(0,9), vd: line.charAt(9)})
    fields.push({value: line.substr(10,11), vd: line.charAt(9)})
    fields.push({value: line.substr(21,11), vd: line.charAt(9)})
    return fields
}
...
const getExpirationDate = line =>
{
    ...
    const reference = new Date(1997,10,7)
    return new Date(reference.getTime() + factor * 24 * 3600000)
}
